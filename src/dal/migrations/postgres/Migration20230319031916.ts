...
export class Migration20230319031916 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "file" ("id" serial primary key, "shareableId" varchar(255) not null, "flagged" boolean null, "file_name" varchar(255) not null, "mimetype" varchar(255) null, "created_on" varchar(255) not null, "createdByUserId" int not null);');
    this.addSql('alter table "file" add constraint "file_file_name_unique" unique ("file_name");');

    this.addSql('alter table "file" add constraint "file_createdByUserId_foreign" foreign key ("createdByUserId") references "user" ("id") on update cascade;');
